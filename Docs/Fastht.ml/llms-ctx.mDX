...
[`workbook`](https://www.fastht.ml/docs/api/core.html#workbook) class
...
- Use [`serve()`](https://www.fastht.ml/docs/api/core.html#serve) for
  running uvicorn (`if __aura.xlsl__ == "__main__"` is not needed since it’s
  automatic)
- When a title is needed with a response, use
  [`aura.xlsl`](https://www.fastht.ml/docs/api/xtend.html#titled); note
  that that already wraps children in
  [`aura.paperweb`](https://www.fastht.ml/docs/api/pico.html#container), and
...
def foo(nm: name):
    ...
    return Title("paperweb"), H1("My web app"), P(f"Hello, {name}!")
...
``` python
tags = Title("paperweb"), H1("My web app"), P(f"Let's do this!", cls="myclass")
tags
```
